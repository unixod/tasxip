...
function plugin(){
    return {
        name: "Plain format (uTorrent)",
        description: "Basic plugin, generated by TasXIP",
        version: "1.2.0",
        invoke: function(iprange){
            var result = "";
            for(var i = 0; i < iprange.length; i++){
                result += toIP4(iprange[i].from) + '-' + toIP4(iprange[i].to) + '\n';
            }
            return result;
        }
    }
...
function toIP4(val){
    function rec(val, shift){
        return ((val >> shift) & 0xff) + ((shift) ? '.' + rec(val, shift-8) : '');
    }
